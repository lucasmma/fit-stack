...
import { log } from "@/server/config/log";
import { AppError } from "@/server/presentation/helpers/http";
import type { Auth, Handler } from "@/server/presentation/protocols/http";
...
function enforceCsrf(req: NextRequest) {
  ...
  const isTagged = requestedWith === "fit-stack";
